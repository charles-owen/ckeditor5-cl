...
export function parseHtml( htmlString ) {
	...
	const normalizedHtml = cleanContentAfterBody( htmlString );

	// Parse htmlString as native Document object.
	const htmlDocument = domParser.parseFromString( normalizedHtml, 'text/html' );

	normalizeSpacerunSpans( htmlDocument );
	convertParagraphs(htmlDocument);
...
/**
 * Convert paragraphs into a single paragraph with break statements between.
 *
 * Until CKeditor gets better at handling pastes from Work and Visual Studio,
 * at least this prevents it from double-spacing everything
 *
 * @param htmlDocument
 */
function convertParagraphs( htmlDocument ) {
	let text = '';

	// Add a break tag to the end of every paragraph. This makes
	// CKEditor single space for some reason
	htmlDocument.querySelectorAll( 'p' ).forEach( el => {
		text += el.innerHTML + '<br>';
	} );
